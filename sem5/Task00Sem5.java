...
*/
public class Task00Sem5 {
    public static void main(String[] args) {
        HashMap<Integer,String> passport = new HashMap<>();
        ...
        for (Map.Entry<Integer,String> entry: passport.entrySet()) {
            if (entry.getValue().equals("Иванов")){
                System.out.println(entry.getValue()+" паспорт " + entry.getKey());
            }
        }
        //или так - по ключам
        for (Integer key: passport.keySet()) {
            if (passport.get(key).equals("Иванов")){
                System.out.println(key + " " + passport.get(key));
            }
        }
    }
}
